#!/usr/bin/env python3
"""Tests for anonymous pre-materialization pull-request binding."""
...
def bind(responses: dict[str, Any]) -> None:
...
class CandidatePrBindingTests(unittest.TestCase):
    def test_open_current_pull_request_is_accepted(self) -> None:
        bind(fixture())
    ...
    def test_every_mutable_binding_rejects_drift(self) -> None:
        ...
        mutations = {
            ...
            "closed pull": lambda value: value[pull_path].__setitem__(
                "state", "closed"
            ),
    ...
    def test_inputs_are_bounded(self) -> None:
        api = FakeApi({})
        invalid = (
            ("../repo", COPIED_REF, HEAD, BASE),
            (REPOSITORY, "pull-request/0", HEAD, BASE),
            (REPOSITORY, COPIED_REF, "not-a-sha", BASE),
            (REPOSITORY, COPIED_REF, HEAD, "A" * 40),
        )
        for values in invalid:
            with self.subTest(values=values):
                with self.assertRaises(binder.BindingError):
                    binder.bind_candidate_pr(api, *values)
