...
if [[ "${profile}" == 'protected-ci' ]]; then
  if [[ "${repository_kind}" == 'spec' && "${runner_os}" != 'Linux' ]]; then
    echo 'spec protected candidate validation requires Linux' >&2
    exit 1
  fi
...
if [[ "${profile}" != 'controller' ]]; then
  ...
  run_prefetch_container "${candidate_image}" \
    ...
    --manifest-path /candidate/conformance/rebuild-rs/Cargo.toml \
    --locked check sources
  run_prefetch_container "${candidate_image}" \
