    ...
    def test_terminal_driver_requires_adopted_cargo_configuration(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for checkout in (policy, candidate):
                ...
                (checkout / "conformance" / ".cargo").mkdir(parents=True)
                (checkout / "conformance" / ".cargo" / "config").write_text(
                    "[net]\noffline=true\n", encoding="utf-8"
                )
                (checkout / "conformance" / "rebuild-rs" / ".cargo").mkdir(
                    parents=True
                )
                (
                    checkout
            ...
            (candidate / "conformance" / ".cargo" / "config").write_text(
