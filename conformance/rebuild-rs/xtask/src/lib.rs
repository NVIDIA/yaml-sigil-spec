...
//! Development and maintenance commands for the specification repository.
...
/// Execute a parsed development command.
///
/// # Errors
///
/// Returns an error when a prerequisite is unavailable, a selected check fails,
/// a candidate path is redirected, or a maintenance operation fails.
pub fn run(cli: Cli) -> io::Result<()> {
    use cli::Task;
    let mut runner = process::SystemRunner;
    match cli.task {
        Task::Check(args) => {
            let selected = args.selected()?;
            let roots = pin_ci_roots(args.candidate_root.clone())
                .map_err(|message| io::Error::new(io::ErrorKind::InvalidInput, message))?;
            ci::run(
                roots.repository.path(),
                roots.rebuild_workspace.path(),
                &selected,
                &args.features,
                &mut runner,
            )
        }
        Task::CandidatePreflight { candidate_root } => {
            let bytes = candidate_preflight(&candidate_root)?;
            println!("candidate ACVP preflight passed ({bytes} bytes)");
            Ok(())
        }
        Task::UpdateAcvp { commit } => update_acvp(&commit),
        Task::Coverage(args) => coverage::run(&workspace_root(), &args, args.open, &mut runner),
        Task::CoverageOpen(args) => coverage::run(&workspace_root(), &args, true, &mut runner),
        Task::Image(args) => image::run(&repository_root(), &args, &mut runner),
    }
}

#[cfg(test)]
fn parse_update_args<I: Iterator<Item = String>>(args: I) -> Result<String, String> {
    use clap::Parser;
    let cli = Cli::try_parse_from(
        ["xtask".to_owned(), "update-acvp".to_owned()]
            .into_iter()
            .chain(args),
    )
    .map_err(|error| error.to_string())?;
    match cli.task {
        cli::Task::UpdateAcvp { commit } => Ok(commit),
        _ => unreachable!(),
    }
}
...
fn pin_ci_roots(candidate: Option<PathBuf>) -> Result<CiRoots, String> {
    let candidate = candidate.unwrap_or_else(repository_root);
...
fn parse_ci_root(args: &[String]) -> Result<CiRoots, String> {
    use clap::Parser;
    let cli = Cli::try_parse_from(["xtask".to_owned(), "check".to_owned()].iter().chain(args))
        .map_err(|error| error.to_string())?;
    match cli.task {
        cli::Task::Check(args) => pin_ci_roots(args.candidate_root),
        _ => unreachable!(),
    }
}
...
fn parse_candidate_preflight_root(args: &[String]) -> Result<PathBuf, String> {
    use clap::Parser;
    let cli = Cli::try_parse_from(
        ["xtask".to_owned(), "candidate-preflight".to_owned()]
            .iter()
            .chain(args),
    )
    .map_err(|error| error.to_string())?;
    match cli.task {
        cli::Task::CandidatePreflight { candidate_root } => Ok(candidate_root),
        _ => unreachable!(),
    }
}
