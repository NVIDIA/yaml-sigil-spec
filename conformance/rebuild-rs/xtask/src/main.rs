...
//!
//! Use the `cargo xtask` alias from `.cargo/config.toml`:
//!
//! ```text
//! cargo xtask ci
...
//! ```
//!
//! `ci` runs repository checks. `candidate-preflight` bounds the candidate's
//! ACVP corpus before candidate execution. `update-acvp` refreshes the pinned
//! NIST ACVP-Server ECDSA SigGen vectors with bounded HTTPS requests.
...
/// Use the parent of `CARGO_MANIFEST_DIR` as the workspace root.
/// Fall back to the current directory when that variable is unavailable.
