...
//! number above the protobuf range, the two invalid wire types, an
//! overflowing tag varint, and an oversized and truncated length. These byte
//! sequences are original negative fixtures generated by this repository.
...
pub fn generate(dir: &PinnedDir) -> std::io::Result<()> {
    ...
    // 11. A length-delimited unknown field declares 2^32 + 5 octets
    //     but supplies one. Its declared length exceeds the remaining
    //     input.
    let mut f11 = f1.clone();
    f11.extend(tag(5, 2));
    f11.extend(varint((1_u64 << 32) + 5));
    f11.push(b'p');
    write_bytes(dir, "oversized-length.binpb", &f11)?;

    // 12–13. Wire types 6 and 7 are not protobuf wire types.
